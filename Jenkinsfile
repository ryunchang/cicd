...
def imageName = "ryunchang/cicd"

def TAG = getTag(gitBranch)

podTemplate(label: label, serviceAccount: 'jenkins-admin', namespace: 'edu13',
    containers: [
    ...
    volumes: [
        ...
        persistentVolumeClaim(mountPath: '/var/jenkins_home', claimName: 'jenkins-edu13-slave-pvc',readOnly: false)
        ]){    
    //node("podman-agent") {
    ...
    node(label) {
       stage('Clone Git Project') {
            sh "pwd"
            echo 'Clone'
            git branch: 'master', credentialsId: 'github_ci', url: 'https://github.com/ryunchang/cicd.git'
            sh "ls"
        }
